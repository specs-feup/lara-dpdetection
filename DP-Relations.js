...
function testInterfaceJp() {

	println(Query.search("interface").get().length);
	...
	for(var $interface of Query.search("interface")) {
		...
		if (methods.length != 0) 		println("    (m) - " + methods.map(m => m.name));

		var allMethods = $interface.allMethods;
		if (allMethods.length != 0) 		println("(all m) - " + allMethods.map(m => m.name));
	}
}
...
function RelationCalls() {
	...
	for(var $class of Query.search("class")) {
		...
		for(var $call of $calls) {
			...
			if ($call.method == undefined) continue;
