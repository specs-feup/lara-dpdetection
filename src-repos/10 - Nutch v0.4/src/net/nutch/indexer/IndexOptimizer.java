...
public class IndexOptimizer {
  ...
  private static class FilterTermDocs implements TermDocs {
    ...
    public void seek(TermEnum enum1) throws IOException { in.seek(enum1); }
  ...
  private static class OptimizingTermPositions extends FilterTermPositions {
    ...
    public void seek(TermEnum enum1) throws IOException {
      super.seek(enum1);
      termDocs.seek(enum1);

      byte[] norms = reader.norms(enum1.term().field());

      sdq.clear();
      float minScore = 0.0f;
      int count = (int)Math.ceil(enum1.docFreq() * FRACTION);
      System.out.println("Optimizing " + enum1.term()
                         + " from " + enum1.docFreq() 
                         + " to " + count); 
      while (termDocs.next()) {
